...
{   
    ...
    public $dbSettigns2 = array(
        'host' => 'localhost',
        'port' => 3306,
        'user' => 'root',
        'password' => '',
        'database' => 'schuleintern_installer'
    );
    ...
    function list_tables_1 ($dbSettigns) {


        $this->DB1 = new mysqli($dbSettigns['host'], $dbSettigns['user'], $dbSettigns['password'], $dbSettigns['database']);
        if ($this->DB1->connect_errno) {
            echo "Failed to connect to MySQL: (" . $this->DB1->connect_errno . ") " . $this->DB1->connect_error;
        }

        $sql = "SHOW TABLES FROM ".$dbSettigns['database']."";
    ...
    function list_tables_2 ($dbSettigns) {


        $this->DB2 = new mysqli($dbSettigns['host'], $dbSettigns['user'], $dbSettigns['password'], $dbSettigns['database']);
        if ($this->DB2->connect_errno) {
            echo "Failed to connect to MySQL: (" . $this->DB2->connect_errno . ") " . $this->DB2->connect_error;
        }
    ...
    function index()
    {
         ...
         */
        $development_tables = $this->list_tables_1($this->dbSettigns);
        $live_tables = $this->list_tables_2($this->dbSettigns2);
    ...
    function manage_tables($tables, $action)
    {
        ...
        if ($action == 'create')
        {
            ...
            {
                // echo "SHOW CREATE TABLE `$table` -- create tables";

                $query = $this->DB1->query("SHOW CREATE TABLE `$table` -- create tables");
                
                $table_structure = $query->fetch_assoc();
    ...
    function compare_table_structures($development_tables, $live_tables)
    {
        ...
        foreach ($development_tables as $table)
        {
            ...
            $table_structure = $query->fetch_array();
