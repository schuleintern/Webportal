<template>
  <div class="">
    ...
    <button v-if="acl.write == 1" class="si-btn margin-r-l" @click="handlerOpen()"><i class="fa fa-plus"></i> Hinzufügen</button>
    ...
    <table class="si-table si-table-style-allLeft" v-if="sortList && sortList.length >= 1">
      ...
      <tr v-bind:key="index" v-for="(item, index) in  sortList" class="">
        <td>
          <div class="" :style="'width: 3rem;height: 3rem;border-radius:3rem;background-color:'+item.color"></div>
        </td>
        <td><a :href="'#item'+item.id" v-on:click="handlerOpen(item)">{{ item.title }}</a></td>
        <td><span v-if="item.leader">{{item.leader.name}}</span></td>
        <td>{{ item.id }}</td>
      </tr>
      </tbody>
    </table>
