...
class extGanztagsModelGroups
{
    ...
    public function getCollection($full = false)
    {
        ...
        if ($full == true) {
            ...
            if ($this->week && count($this->week) > 0) {
                $collection['week'] = [];
                foreach ($this->week as $day => $week) {
                    if ($week) {
                        $collection['week'][$day][] = $week->getCollection();
                    }
                }
            }
        }
     ...
     * @return Array[]
     */
    public function getSchueler($day = false)
    {
        if (!$this->getID()) {
            return false;
        }
            ...
            if ($data['days']) {
                ...
                foreach ($this->week as $day => $week) {
                    if ($days->{$day} && $days->{$day}->group == $this->getID()) {
                        $this->week[$day] = new extGanztagsModelSchueler($data, user::getUserByID($data['user_id']));
                    }
                }
            }
        }
        ...
        foreach ($items as $item) {
            ...
            if ($item->id) {
                ...
                if (!DB::getDB()->query("INSERT INTO ext_ganztags_groups
                    (
                        type, title, leader_id, room, color, info, days, duration, leader_id
                    ) values(
                             'group',
