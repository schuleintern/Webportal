<template>
  <div class="">
    ...
    <select class="si-input margin-l-m" v-on:change="handlerFilter($event, 'day')">
    ...
    <button v-if="acl.write == 1" class="si-btn margin-l-l" @click="handlerOpen()"><i class="fa fa-plus"></i> Aufsicht hinzufügen</button>
    ...
    <table class="si-table si-table-style-allLeft" v-if="sortList && sortList.length >= 1">
      <thead>
      <tr>
        <th v-on:click="handlerSort('dayTitle')" class="curser-sort" :class="{'text-orange': sort.column == 'dayTitle'}">Tag</th>
        <th v-on:click="handlerSort('userName')" class="curser-sort" :class="{'text-orange': sort.column == 'userName'}">Aufsicht</th>
        <th v-on:click="handlerSort('secondName')" class="curser-sort" :class="{'text-orange': sort.column == 'secondName'}">Vertretung</th>
        <th >Pause</th>
        <th ></th>
      </tr>
      </thead>
      ...
      <tr v-bind:key="index" v-for="(item, index) in  sortList" class="">
        ...
        <td><a v-if="acl.write == 1" :href="'#item'+item.id" v-on:click="handlerOpen(item)">{{ item.dayTitle }}</a>
        <span v-else>{{ item.dayTitle}}</span></td>
        <td>{{ item.userName }}</td>
        <td><span v-if="item.secondName">{{ item.secondName }}</span></td>
        <td><span v-if="item.pause">{{ item.pause.title }}</span></td>
        <td><span v-if="item.pause">{{item.pause.start}} - {{item.pause.end}}</span></td>
      </tr>
      </tbody>
    </table>
    <div v-else>
...
export default {
  ...
  data() {
    return {
      ...
      searchColumns: ['id','userName','day', 'secondName'],
      searchString: '',
      filter: {
        colum: false,
        value: false
