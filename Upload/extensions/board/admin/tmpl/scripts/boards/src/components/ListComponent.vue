  ...
  <div class="">
    ...
    <table class="si-table si-table-style-allLeft" v-if="sortList && sortList.length >= 1">
      ...
      <tr>
        ...
        <th v-on:click="handlerSort('catTitle')" class="curser-sort"
            :class="{'text-orange': sort.column == 'catTitle'}">Kategory
        </th>
        <th v-if="filter.colum == 'catTitle'" v-on:click="handlerSort('sort')" class="curser-sort" :class="{'text-orange': sort.column == 'sort'}">
          Sortierung
        </th>
      ...
      <draggable v-model="sortList" tag="tbody" handle=".sortHandle" group="sort" @start="drag=true" @end="drag=false">
        <template #item="{element}">
          <tr>
            <td>{{ element.title }}</td>
            <td>{{ element.catTitle }}</td>
            <td v-if="filter.colum == 'catTitle'">
              <button class="sortHandle si-btn si-btn-icon" :class="{'si-btn-border': sort.column != 'sort'}"> {{ element.sort }} <i class="fas fa-sort"></i></button>
            </td>
            <td>
              <FormToggle :disable="false" :input="element.state"></FormToggle>
            </td>
            <td>
              <button class="si-btn margin-l-l" @click="handlerEdit(element)"><i class="fa fa-list"></i> Elemente
              </button>
            </td>
