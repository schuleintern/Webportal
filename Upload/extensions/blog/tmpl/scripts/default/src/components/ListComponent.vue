<template>
  <div class="">
    ...
    <table class="si-table si-table-style-allLeft" v-if="sortList && sortList.length >= 1">
      <thead>
        <tr>
          <th v-on:click="handlerSort('dueDate')" class="curser-sort" :class="{'text-orange': sort.column == 'dueDate'}">Datum</th>
      ...
      <tbody>
        <tr v-bind:key="index" v-for="(item, index) in  sortList" class="">
          ...
          <td>{{ item.amount }}</td>
          <td>
            <button v-if="item.state == 1" class="si-btn si-btn-off margin-r-m"><i class="fa fa-check"></i> Offen</button>
            <button v-if="item.state == 2" class="si-btn si-btn-off text-green margin-r-m"><i class="fa fa-check"></i> Bezahlt</button>
            <!--
            <a v-if="item.state == 1" href="index.php?page=ext_finanzen&view=default&task=transfer" class="si-btn"><i class="fa fa-check"></i> Jetzt Bezahlen</a>-->
          </td>
          <td>
            <button v-if="item.state == 1" class="si-btn margin-r-m" @click="handlerDetails(item)">Details</button>
          </td>
        </tr>
      </tbody>
    </table>
