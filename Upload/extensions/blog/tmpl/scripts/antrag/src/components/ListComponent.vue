<template>
  <div class="">
    ...
    <button v-if="acl.write == 1" class="si-btn" @click="handlerAdd()"><i class="fa fa-plus"></i> Hinzufügen</button>
    <table class="si-table" v-if="sortList && sortList.length >= 1">
      <thead>
        <tr>
          ...
          <th v-on:click="handlerSort('state')" class="curser-sort" :class="{'text-orange': sort.column == 'dueDate'}">Status</th>
          <th></th>
        </tr>
      </thead>
      <tbody>
        <tr v-bind:key="index" v-for="(item, index) in  sortList" class="">
          ...
          <td>{{ item.dueDate }}</td>
          <td>
            <button v-if="item.state == 1" class="si-btn si-btn-border">Offen</button>
            <button v-if="item.state == 2" class="si-btn si-btn-border">Freigegeben</button>
          </td>
          <td>x% schon fertig</td>
        </tr>
      </tbody>
    </table>
    <div v-else>
