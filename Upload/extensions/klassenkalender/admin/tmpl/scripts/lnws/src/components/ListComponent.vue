  ...
  <div class="">
    ...
    <table class="si-table" v-if="sortList && sortList.length >= 1">
      ...
      <tr>
        ...
        <th v-on:click="handlerSort('short')" class="curser-sort" :class="{'text-orange': sort.column == 'short'}">Kurztitel</th>
        <th  class="curser-sort" >Öffentlich</th>
      ...
      <tr v-bind:key="index" v-for="(item, index) in  sortList" class="">
        ...
        <td>{{ item.short }}</td>
        <td><FormToggle :input="item.isPublic" :disable="0"></FormToggle></td>
        <td>{{ item.id }}</td>
      </tr>
      </tbody>
    </table>
