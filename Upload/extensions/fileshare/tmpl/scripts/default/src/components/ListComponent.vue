  ...
  <div class="">
    ...
    <table class="si-table " v-if="sortList && sortList.length >= 1">
      ...
      <tr v-bind:key="index" v-for="(item, index) in  sortList" class="">
        <td class=""><a :href="'#item'+item.id" v-on:click="handlerOpen(item)">{{ item.title }}</a></td>
        <td>
          <button class="si-btn" @click="handlerDownload(item)"><i class="fa fa-download"></i> Download</button>
        </td>
