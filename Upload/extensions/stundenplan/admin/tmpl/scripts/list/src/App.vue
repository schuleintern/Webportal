...
export default {
  ...
  created() {
    ...
    this.$bus.$on('item--submit', data => {
      ...
      formData.append('start', data.item.start);
      formData.append('end', data.item.end);

      this.loading = true;
      var that = this;
      axios.post(this.apiURL + '/setStundenplaene', formData)
  ...
  methods: {
    ...
    loadList() {
      ...
      this.loading = true;
      var that = this;
      axios.get(this.apiURL + '/getAdminStundenplaene')
      .then(function (response) {
        if (response.data) {
          if (response.data.error) {
            that.error = '' + response.data.msg;
...
:root:root {
  --dp-font-size: 11pt;
  --dp-action-button-height: 20px;
  --dp-preview-font-size: 9pt;
  --dp-action-buttons-padding: 0.6rem
}
</style>
