  ...
  <div class="">
    ...
    <table class="si-table si-table-style-allLeft" v-if="sortList && sortList.length >= 1">
      ...
      <tr>
        ...
        <th v-on:click="handlerSort('start')" class="curser-sort" :class="{'text-orange': sort.column == 'start'}">Start</th>
        <th v-on:click="handlerSort('end')" class="curser-sort" :class="{'text-orange': sort.column == 'end'}">Ende</th>
        <th v-on:click="handlerSort('createdBy')" class="curser-sort" :class="{'text-orange': sort.column == 'createdBy'}">Von</th>
      ...
      <tr v-bind:key="index" v-for="(item, index) in  sortList" class="">
        ...
        <td><a :href="'index.php?page=ext_stundenplan&view=plan&admin=true&id='+item.id" >{{ item.title }}</a></td>

        <td>{{ item.start }}</td>
        <td>{{ item.end }}</td>
        <td><User v-if="item.createdBy" :data="item.createdBy"></User></td>
        <td>{{ item.id }}</td>
      </tr>
      </tbody>
    </table>
