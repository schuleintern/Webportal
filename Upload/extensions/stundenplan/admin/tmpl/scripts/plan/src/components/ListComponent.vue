<template>
  <div class="">
    ...
    <table class="si-table si-table-style-allLeft" v-if="sortList && sortList.length >= 1">
      <thead>
      <tr>
        <th v-on:click="handlerSort('day')" class="curser-sort" :class="{'text-orange': sort.column == 'day'}">Tag</th>
        <th v-on:click="handlerSort('hour')" class="curser-sort" :class="{'text-orange': sort.column == 'hour'}">Stunde</th>
        <th v-on:click="handlerSort('teacher')" class="curser-sort" :class="{'text-orange': sort.column == 'teacher'}">Lehrer*in</th>
        <th v-on:click="handlerSort('fach')" class="curser-sort" :class="{'text-orange': sort.column == 'fach'}">Fach</th>
        <th v-on:click="handlerSort('klasse')" class="curser-sort" :class="{'text-orange': sort.column == 'klasse'}">Klasse</th>
        <th v-on:click="handlerSort('room')" class="curser-sort" :class="{'text-orange': sort.column == 'room'}">Raum</th>
      </tr>
      </thead>
      <tbody>
      <tr v-bind:key="index" v-for="(item, index) in  sortList" class="">
        <td><a :href="'#'+item.id" @click="handlerOpen(item)">{{ item.dayLabel }}</a></td>
        <td>{{ item.hour }}</td>
        <td>{{ item.teacher }}</td>
        <td>{{ item.fach }}</td>
        <td>{{ item.klasse }}</td>
...
export default {
  ...
  data() {
    return {
      ...
      searchColumns: ['fach', 'klasse','raum'],
      searchString: '',
      filter: {
        colum: false,
        value: false
