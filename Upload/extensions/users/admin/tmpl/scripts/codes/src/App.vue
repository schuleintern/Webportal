...
export default {
  ...
  created() {
    ...
    this.$bus.$on('item--submit', data => {
      ...
      if (!data.schueler || !data.schueler.id) {
        console.log('missing');
        return false;
      }

      const formData = new FormData();
      formData.append('userID', data.schueler.id);

      this.loading = true;
      var that = this;
      axios.post(this.apiURL + '/setCodes', formData)
    ...
    this.$bus.$on('item--print', data => {

      if (!data.codeSchuelerAsvID) {
        console.log('missing');
        return false;
      }

      const formData = new FormData();
      formData.append('asvID', data.codeSchuelerAsvID);

      this.loading = true;
      var that = this;
      axios.post(this.apiURL + '/getCodePrint', formData)
      .then(function (response) {
        if (response.data) {
