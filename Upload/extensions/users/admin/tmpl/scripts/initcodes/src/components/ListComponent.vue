<template>
  <div class="">
    ...
    <table class="si-table si-table-style-allLeft" v-if="sortList && sortList.length >= 1">
      <thead>
      <tr>
        <th v-on:click="handlerSort('initialPasswordID')" class="curser-sort" :class="{'text-orange': sort.column == 'initialPasswordID'}">
          ID
        </th>
        <th v-on:click="handlerSort('userName')" class="curser-sort"
            :class="{'text-orange': sort.column == 'userName'}">Benutzer
        </th>
        <th v-on:click="handlerSort('initialPassword')" class="curser-sort"
            :class="{'text-orange': sort.column == 'initialPassword'}">Code
        </th>
        <th v-on:click="handlerSort('passwordPrinted')" class="curser-sort"
            :class="{'text-orange': sort.column == 'passwordPrinted'}">Gedruckt
        </th>
        <th></th>
      ...
      <tr v-bind:key="index" v-for="(item, index) in  sortList" class="">
        <td>{{ item.initialPasswordID }}</td>
        <td>
          <User v-if="item.initialPasswordUserID && item.user" :data="item.user"></User>
          <span v-else>Benutzer gelöscht: UserID {{item.initialPasswordUserID}}</span>
        </td>
        <td>{{ item.initialPassword }}</td>
        <td>{{ item.passwordPrintedTime }}</td>
        <td>
          <a v-if="item.initialPasswordUserID && item.user" class="si-btn" :href="'index.php?page=ext_users&view=initcodes&admin=true&task=print&u='+item.initialPasswordUserID" ><i class="fa fa-print"></i>Drucken</a>
        </td>
      </tr>
      </tbody>
    </table>
...
export default {
  ...
  data() {
    return {
      ...
      sort: {
        column: 'passwordPrinted',
        order: false
      },
      searchColumns: ['initialPasswordID', 'passwordPrintedTime', 'userName'],
      searchString: '',
      filter: {
        colum: false,
        value: false
