    ...
    <table class="si-table" v-if="sortList && sortList.length >= 1">
      ...
      <tbody>
        <tr v-bind:key="index" v-for="(item, index) in  sortList" class="">
          ...
          <td><a :href="'#item' + item.id" v-on:click="handlerOpen(item)">{{ item.user.name }} ({{ item.user.klasse }})</a></td>

          <td>

            <button v-if="item.state == 1" class="si-btn si-btn-green" v-on:click="handlerSet(item)"><i class="fa fa-check"></i>
              Freigeben</button>
