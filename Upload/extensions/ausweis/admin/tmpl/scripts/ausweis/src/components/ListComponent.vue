<template>
  <div class="">
    ...
    <input type="text" class="si-input" v-model="searchString" placeholder="Suche..." />
    ...
    <table class="si-table" v-if="sortList && sortList.length >= 1">
      ...
      <tbody>
        <tr v-bind:key="index" v-for="(item, index) in  sortList" class="">
          ...
          <td>{{ item.userName }}</td>
          <td><a :href="selfURL + '&task=getFile&path=' + item.front_path" target="_blank"><img
                :src="selfURL + '&task=getFile&path=' + item.front_path" width="100" /></a></td>
          <td>
            <button v-if="item.state == 1" class="si-btn si-btn-red" v-on:click="handlerSet(item, 2)"><i
                class="fa fa-check"></i>
              Sperren</button>
            <div v-if="item.state == 2">
              <b>- gesperrt -</b><br>
              <button class="si-btn si-btn-green margin-r-m " v-on:click="handlerSet(item, 1)"><i class="fa fa-check"></i>
                Freigeben</button>
              <button class="si-btn si-btn-red " v-on:click="handlerDelete(item)"><i class="fa fa-trash"></i>
                Löschen</button>
            </div>
          </td>
