...
export default {
  ...
  created() {
    ...
    this.$bus.$on('item--delete', data => {
      ...
      this.loading = true;
      var that = this;
      axios.post(this.apiURL + '/deleteAusweisAdmin', formData)
    ...
    this.$bus.$on('item--setState', data => {

      if (!data.item.id || !data.state) {
        console.log('missing');
        return false;
      }
      ...
      this.loading = true;
      var that = this;
      axios.post(this.apiURL + '/setAusweisAdmin', formData)
        .then(function (response) {
          if (response.data) {
  ...
  methods: {
    ...
    loadList() {
      ...
      this.loading = true;
      var that = this;
      axios.get(this.apiURL + '/getAusweisAdmin')
        .then(function (response) {
          if (response.data) {
            if (response.data.error) {
              that.error = '' + response.data.msg;
