<template>
  ...
  <div>
    ...
    <div class="si-hinweis text-red" v-if="suggest != 1">
      Diese Funktion ist deaktiviert!<br>
      <a href="index.php?page=ext_kalender&view=settings&admin=true">Zu den Einstellungen</a>
    </div>
...
export default {
  ...
  methods: {
    ...
    loadList() {
      ...
      this.loading = true;
      var that = this;
      axios.get(this.apiURL + '/getAdminEventsByStatus')
      .then(function (response) {
        if (response.data) {
          if (response.data.error) {
            that.error = '' + response.data.msg;
