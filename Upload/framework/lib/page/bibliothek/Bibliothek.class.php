...
class Bibliothek extends AbstractPage {

  public function __construct() {
    parent::__construct(['Bibliothek']);
  ...
  private function rueckgabe() {
              ...
              if($ausleihen[$i]->isAusleiheActive()) {
                  if($_REQUEST['deleteAusleihe'] == $ausleihen[$i]->getID()) {
                      ...
                      $buecherHTML .= "<td>
                                ...
                                <button type=\"button\" class=\"btn btn-xs btn-danger\" onclick=\"confirmAction('Ausleihe wirklich löschen?','index.php?page=Bibliothek&mode=rueckgabe&schuelerAsvIDPreSet=" . $schueler->getAsvID() . "&deleteAusleihe=" . $ausleihen[$i]->getID() . "')\"><i class=\"fa fa-trash\"></i> Löschen</button>
      ...
      eval("DB::getTPL()->out(\"" . DB::getTPL()->get("bibliothek/rueckgabe/index") . "\");");
  }
  ...
  private function showAllAusleihen() {
      ...
      for($i = 0; $i < sizeof($klassen); $i++) {
          ...
          eval("\$html .= \"" . DB::getTPL()->get("bibliothek/management/ausleihen_klasse") . "\";");
                    
          
      }
      
      eval("DB::getTPL()->out(\"" . DB::getTPL()->get("bibliothek/management/ausleihen") . "\");");
      PAGE::kill(true);
			//exit(0);
  ...
  private function viewExemplar() {
    ...
    if($exemplar != null) {
      ...
      eval("DB::getTPL()->out(\"" . DB::getTPL()->get("bibliothek/exemplar/index") . "\");");
  ...
  private function management() {
    ...
    eval("DB::getTPL()->out(\"" . DB::getTPL()->get("bibliothek/management/index") . "\");");
    PAGE::kill(true);
			//exit(0);
  }
  ...
  private function deleteBook() {
    ...
    header("Location: index.php?page=Bibliothek&mode=bestand");
    exit(0);
  ...
  private function deleteExemplar() {
    ...
    if($exemplar != null) {
      $exemplar->delete();
      header("Location: index.php?page=Bibliothek&mode=bestandOfBook&schulbuchID=" . $exemplar->getSchulbuch()->getID());
    }
    else {
      header("Location: index.php?page=Bibliothek&mode=bestand");
      exit(0);
  ...
  private function ausleihe() {
      ...
      for($i = 0; $i < sizeof($ausleihen); $i++) {
        if($_REQUEST['deleteAusleihe'] == $ausleihen[$i]->getID()) {
          ...
          $buecherHTML .= "<td>
                                ...
                                <button type=\"button\" class=\"btn btn-xs btn-danger\" onclick=\"confirmAction('Ausleihe wirklich löschen?','index.php?page=Bibliothek&mode=ausleihe&schuelerAsvID=" . $schueler->getAsvID() . "&deleteAusleihe=" . $ausleihen[$i]->getID() . "')\"><i class=\"fa fa-trash\"></i> Löschen</button>
    ...
    eval("DB::getTPL()->out(\"" . DB::getTPL()->get("bibliothek/ausleihe/index") . "\");");
  }
  ...
  private function bestandOfBook() {
    ...
    for($i = 0; $i < sizeof($exemplare); $i++) {
      ...
      $exemplarHTML .= "<td><form><button type=\"button\" class=\"btn\" onclick=\"javascript:confirmAction('Soll das Buch wirklich gelöscht werden?','index.php?page=Bibliothek&mode=deleteExemplar&exemplarID=" . $exemplare[$i]->getID() . "')\"><i class=\"fa fa-trash\"></i></button></form></td>";
      $exemplarHTML .= "</tr>";

      $exemplareHTML .= $exemplarHTML;
    }

    eval("DB::getTPL()->out(\"" . DB::getTPL()->get("bibliothek/bestand/buch") . "\");");
  }
  ...
  private function bestand() {
    ...
    for($i = 0; $i < sizeof($books); $i++) {
      ...
      $html .= "<td><a href=\"index.php?page=Bibliothek&mode=bestandOfBook&schulbuchID=" . $books[$i]->getID() . "\"><b>" . $books[$i]->getName() . "</b></a></td>";
      ...
      $html .= "<button type=\"button\" class=\"btn btn-info btn-xs\" onclick=\"javascript:window.location.href='index.php?page=Bibliothek&mode=bestandOfBook&schulbuchID=" . $books[$i]->getID() . "'\"><i class=\"fa fa-briefcase\"></i> Bestand verwalten</button><br />";
      $html .= "<button type=\"button\" class=\"btn btn-danger btn-xs\" onclick=\"javascript:confirmAction('Soll das Buch inkl. allen Exemplaren und allen aktiven Ausleihen gelöscht werden?','index.php?page=Bibliothek&mode=deleteBook&schulbuchID=" . $books[$i]->getID() . "')\"><i class=\"fa fa-trash\"></i> Buch löschen</button><br />";
    ...
    eval("DB::getTPL()->out(\"" . DB::getTPL()->get("bibliothek/bestand/index") . "\");");
  }
  ...
  private function showMyBooks() {
    ...
    eval("DB::getTPL()->out(\"" . DB::getTPL()->get("bibliothek/mybooks/index") . "\");");
  ...
  public static function userCanAusleihe($user) {
    ...
    if($user->isMember('Webportal_Bibliothek_Ausleihe')) return true;
    if($user->isMember('Webportal_Bibliothek_Admin')) return true;
  ...
  public static function userCanRueckgabe($user) {
    if($user->isAdmin()) return true;
    if($user->isMember('Webportal_Bibliothek_Rueckgeber')) return true;
    if($user->isMember('Webportal_Bibliothek_Admin')) return true;
  ...
  public static function userCanBestand($user) {
    if($user->isAdmin()) return true;
    if($user->isMember('Webportal_Bibliothek_Bestand')) return true;
    if($user->isMember('Webportal_Bibliothek_Admin')) return true;
  ...
  public static function displayAdministration($selfURL) {
    ...
    $currentAusleiher = administrationmodule::getUserListWithAddFunction($selfURL, 'ausleiher', 'addUserAusleiher', 'deleteUserAusleiher', 'Ausleiher der Schulbuchbücherei', 'Hier angegebene Benutzer können Schulbücher ausleihen.', 'Webportal_Bibliothek_Ausleiher');

    $currentRueckgeber = administrationmodule::getUserListWithAddFunction($selfURL, 'rueckgeber', 'addUserRueckgeber', 'deleteUserRueckgeber', 'Rückgeber der Schulbuchbücherei', 'Hier angegebene Benutzer können Schulbücher zurückgeben.', 'Webportal_Bibliothek_Rueckgeber');

    $currentBestandVerwalter = administrationmodule::getUserListWithAddFunction($selfURL, 'bestandverwalter', 'addUserBestand', 'deleteUserBestand', 'Bestandsverwalter der Schulbuchbücherei', 'Hier angegebene Benutzer können den Bestand verwalten.', 'Webportal_Bibliothek_Bestand');

    eval("\$return = \"" . DB::getTPL()->get("bibliothek/admin/index") . "\";");

    return $return;
  }

  public static function getAdminGroup() {
    return 'Webportal_Bibliothek_Admin';
  }
}
