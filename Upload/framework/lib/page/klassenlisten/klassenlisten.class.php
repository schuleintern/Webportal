    ...
    public function execute()
    {
        ...
        if (isset($_REQUEST['grade']) && $_REQUEST['grade'] != "") {
            ...
            if (isset($_REQUEST['createPDF']) && $_REQUEST['createPDF'] != "") {
                ...
                if (count($colPrint) > 0) $perCel = floor((100 - $abzug) / sizeof($colPrint));    // Breite der einzelnen Zellen
                else {
                    $colPrint[] = "&nbsp;";
                    $perCel = 100 - $abzug;
                }
                ...
                $exportClass->setOptions([
                    'title' => 'Klassenliste ' . $grade,
                    'desc' => 'Klassenliste der Klasse ' . $grade,
                ...
                */
                ...
                $excelFile->setActiveSheetIndex(0)->getPageSetup()->setPaperSize(\PhpOffice\PhpSpreadsheet\Worksheet\PageSetup::PAPERSIZE_A4);
                $excelFile->setActiveSheetIndex(0)->getPageSetup()->setOrientation(\PhpOffice\PhpSpreadsheet\Worksheet\PageSetup::ORIENTATION_PORTRAIT);
                ...
                */
                ...
                $sheet->setActiveSheetIndex(0)->setCellValue("A1", "Nummer");
                $sheet->setActiveSheetIndex(0)->setCellValue("B1", "Name");
                $sheet->setActiveSheetIndex(0)->setCellValue("C1", "Vornamen");
                $sheet->setActiveSheetIndex(0)->setCellValue("D1", "Rufname");
                $sheet->setActiveSheetIndex(0)->setCellValue("E1", "Geburtsdatum");

                $sheet->getActiveSheet()->getStyle("A1")->getFont()->setBold(true);
                $sheet->getActiveSheet()->getStyle("B1")->getFont()->setBold(true);
                $sheet->getActiveSheet()->getStyle("C1")->getFont()->setBold(true);
                $sheet->getActiveSheet()->getStyle("D1")->getFont()->setBold(true);
                $sheet->getActiveSheet()->getStyle("E1")->getFont()->setBold(true);


                for ($i = 0; $i < sizeof($pupils); $i++) {
                    $sheet->setActiveSheetIndex(0)->setCellValue("A" . ($i + 2), $i + 1);
                    $sheet->setActiveSheetIndex(0)->setCellValue("B" . ($i + 2), ($pupils[$i]->getName()));
                    $sheet->setActiveSheetIndex(0)->setCellValue("C" . ($i + 2), ($pupils[$i]->getVornamen()));
                    $sheet->setActiveSheetIndex(0)->setCellValue("D" . ($i + 2), ($pupils[$i]->getRufname()));
                    $sheet->setActiveSheetIndex(0)->setCellValue("E" . ($i + 2), ($pupils[$i]->getGeburtstagAsNaturalDate()));
                }

                $sheet->getActiveSheet()->getPageSetup()->setPaperSize(\PhpOffice\PhpSpreadsheet\Worksheet\PageSetup::PAPERSIZE_A4);
                $sheet->getActiveSheet()->getPageSetup()->setOrientation(\PhpOffice\PhpSpreadsheet\Worksheet\PageSetup::ORIENTATION_PORTRAIT);
                ...
                $sheet->getActiveSheet()->getStyle('A1:' . 'E' . (sizeof($pupils) + 1))->applyFromArray($BStyle);
