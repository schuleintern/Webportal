...
class BibliothekBuchAusleihe {
	...
	public function __construct($data, $exemplar=null) {
		...
		else $this->exemplar = BibliothekExemplar::getByID($data['ausleiheExemplarID']);
	...
	public function delete() {
		DB::getDB()->query("DELETE FROM bibliothek_ausleihe WHERE ausleiheID='" . $this->getID() . "'");
	}
	...
	public function setKommentar($text) {
	    DB::getDB()->query("UPDATE bibliothek_ausleihe SET ausleiheKommentar='" . DB::getDB()->escapeString($text) . "' WHERE ausleiheID='" . $this->getID() . "'");
	}
	
	
	/**
	 * 
	 * @param BibliothekExemplar $exemplar
	 * @return BuchAusleihe[]
	 */
	public static function getByExemplar($exemplar) {
		$ausleihen = [];
		
		$ausleihenSQL = DB::getDB()->query("SELECT * FROM bibliothek_ausleihe WHERE ausleiheExemplarID='". $exemplar->getID() . "'");#
		
		while($aus = DB::getDB()->fetch_array($ausleihenSQL)) {
			$ausleihen[] = new BibliothekBuchAusleihe($aus, $exemplar);
		}
	...
	public static function lendExemplarToSchueler($exemplar, $schueler, $startDate = '') {
		...
		DB::getDB()->query("INSERT INTO bibliothek_ausleihe (
				ausleiheExemplarID,
				ausleiheStartDatum,
	...
	public function endAusleihe($endDate='') {
		...
		if($this->isAusleiheActive()) {
			DB::getDB()->query("UPDATE bibliothek_ausleihe SET ausleiheEndDatum='" . $endDate . "', rueckgeberUserID='" . DB::getUserID() . "' WHERE ausleiheID='" . $this->getID() . "'");
		}
	}
	...
	public static function getBySchueler($schueler) {
		...
		for($i = 0; $i < sizeof($schueler); $i++) {
			$data = DB::getDB()->query("SELECT * FROM bibliothek_ausleihe WHERE ausleiherSchuelerAsvID='" . $schueler[$i]->getAsvID() . "' ORDER BY ausleiheStartDatum DESC");
	...
	public static function getActiveAusleiheBySchueler($schueler) {
	    ...
	    for($i = 0; $i < sizeof($schueler); $i++) {
	        $data = DB::getDB()->query("SELECT * FROM bibliothek_ausleihe WHERE ausleiherSchuelerAsvID='" . $schueler[$i]->getAsvID() . "' AND ausleiheEndDatum IS NULL ORDER BY ausleiheStartDatum DESC");
	        while($d = DB::getDB()->fetch_array($data)) {
	            $ausleihe[] = new BuchAusleihe($d);
	        }
	...
	public static function getByLehrer($lehrer) {
		...
		for($i = 0; $i < sizeof($lehrer); $i++) {
			$data = DB::getDB()->query("SELECT * FROM bibliothek_ausleihe WHERE ausleiherLehrerAsvID='" . $lehrer[$i]->getAsvID() . "' ORDER BY ausleiheStartDatum DESC");
			while($d = DB::getDB()->fetch_array($data)) {
				$ausleihe[] = new BuchAusleihe($d);
			}
