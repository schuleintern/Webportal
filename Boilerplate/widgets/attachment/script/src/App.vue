...
export default {
  ...
  data() {
    return {
      apiURL: window.globals_widget_fileshare_attachment.apiURL,
      acl: window.globals_widget_fileshare_attachment.acl,
      randFolder: window.globals_widget_fileshare_attachment.randFolder,
      error: false,
      loading: false,
      page: 'item',
  ...
  created() {
    ...
    this.$bus.$on('item--submit', data => {
      ...
      formData.append('folder', data.item.folder );
